...
export class 求解器类 {
  ...
  constructor(牌库数量元组: number[], 演算奖励元组: number[]) {
    this.牌库数量元组 = 牌库数量元组;
    this.演算奖励元组 = 演算奖励元组;
  }
  ...
  是需要考虑的状态(状态: 过渡态类): boolean {
    ...
    if (!(状态.剩余演算次数 - 1 <= 状态.剩余翻倍次数 && 状态.剩余翻倍次数 <= 2)) return false;

    const 已抽到的铭牌总数量 = 状态.已抽到的铭牌数量元组.reduce((a, b) => a + b, 0);
    if (已抽到的铭牌总数量 <= 1 && 状态.是否翻倍) return false;
    if (已抽到的铭牌总数量 === 2 && 状态.是否翻倍 && 状态.剩余翻倍次数 === 0) return false;
  ...
  构建状态列表(): void {
    ...
    for (let 剩余演算次数 = 1; 剩余演算次数 <= 3; 剩余演算次数++) {
      for (let 剩余放弃次数 = 0; 剩余放弃次数 <= 3; 剩余放弃次数++) {
        for (let 剩余翻倍次数 = 0; 剩余翻倍次数 <= 2; 剩余翻倍次数++) {
...
export const 默认牌库数量元组 = [5, 5, 5, 8, 6];
