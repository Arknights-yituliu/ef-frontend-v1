...
function createBlocks() {
  ...
  if (!hexagonDef) {
    // 如果不存在
    const defs =
      containerRef.value.querySelector('defs') ||
      document.createElementNS('http://www.w3.org/2000/svg', 'defs'); // 获取或创建 defs 元素
...
function animateProgress() {
  ...
  progressTimeline
    ...
    .to(
      [logoRef.value, progressCircleRef.value],
      {
        opacity: 0, // 设置透明度为 0
        duration: Math.max(props.loadingDuration, 0) / 1000, // 动画持续时间，单位转换为秒
        ease: 'power2.out', // 缓动函数
...
const showRevealPhase = computed(
  () => props.isLoading && (stage.value === 'reveal' || stage.value === 'loading'),
);
</script>
        ...
        <div class="logo-container">
          ...
          <div class="progress-ring-container">
            ...
            <svg class="progress-ring" viewBox="0 0 200 200">
              ...
              />
              <!-- 背景圆环 -->
              <circle class="progress-bg" cx="100" cy="100" fill="none" r="80" stroke-width="4" />
              <!-- 进度圆环 -->
              <circle
...
.progress-outer {
  ...
  transition:
    ...
    opacity var(--transition-base); /* 过渡效果 */
}
